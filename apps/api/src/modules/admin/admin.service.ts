...
const definitions:Record<string,{model:string;permission:string;schema:z.ZodTypeAny;include?:object}>={
  faculties:{model:'faculty',permission:'faculties.manage',schema:z.object({code:z.string().trim().min(1).max(20),name:z.string().trim().min(1).max(150),active:z.boolean().default(true)}).strict()},
  departments:{model:'department',permission:'departments.manage',schema:z.object({code:z.string().trim().min(1).max(20),name:z.string().trim().min(1).max(150),facultyId:uuid,active:z.boolean().default(true)}).strict(),include:{faculty:true}},
  courses:{model:'course',permission:'courses.manage',schema:z.object({code:z.string().trim().min(1).max(30),title:z.string().trim().min(1).max(150),departmentId:uuid,creditHours:z.number().int().min(0).max(30),active:z.boolean().default(true)}).strict(),include:{department:true}},
  'academic-years':{model:'academicYear',permission:'academic_periods.manage',schema:z.object({name:z.string().trim().min(1).max(30),startsOn:date,endsOn:date,active:z.boolean().default(true)}).strict()},
  semesters:{model:'semester',permission:'academic_periods.manage',schema:z.object({name:z.string().trim().min(1).max(50),academicYearId:uuid,startsOn:date,endsOn:date,active:z.boolean().default(true)}).strict(),include:{academicYear:true}},
  'workload-rules':{model:'workloadRule',permission:'workloads.manage',schema:z.object({category:z.string().trim().min(1).max(40),expectedWeeklyHours:decimal.refine(value=>Number(value)>0&&Number(value)<=168,'Normal weekly hours must be greater than zero and no more than 168.'),effectiveFrom:date,effectiveTo:date,active:z.boolean().default(true)}).strict()},
  ...
  'payment-rates':{model:'paymentRate',permission:'rates.manage',schema:z.object({category:z.string().trim().min(1).max(40),employmentType:z.enum(['PART_TIME','FULL_TIME']),semesterId:uuid,rate:decimal.refine(v=>Number(v)>0,'Rate must be positive.'),currency:z.string().regex(/^[A-Z]{3}$/),roundingPlaces:z.number().int().min(0).max(4),active:z.boolean().default(true)}).strict(),include:{semester:true}},
};
