...
const definitions:Record<string,{model:string;permission:string;schema:z.ZodTypeAny;include?:object}>={
  ...
  'workload-rules':{model:'workloadRule',permission:'workloads.manage',schema:z.object({category:z.string().min(2).max(40),expectedWeeklyHours:decimal.refine(value=>Number(value)>0&&Number(value)<=168,'Normal weekly hours must be greater than zero and no more than 168.'),effectiveFrom:date,effectiveTo:date,active:z.boolean().default(true)}).strict()},
