...
async function main(){
  ...
  const faculty=await db.faculty.upsert({where:{code:'UPSA-ADMIN'},create:{code:'UPSA-ADMIN',name:'University Administration'},update:{}});
  await db.department.upsert({where:{code:'UPSA-ADMIN'},create:{code:'UPSA-ADMIN',name:'University Administration',facultyId:faculty.id},update:{facultyId:faculty.id,active:true}});
  if(await db.user.count({where:{active:true,roles:{some:{role:{code:'ADMIN'}}}}})){console.log('An active administrator already exists; foundation records verified.');return;}
