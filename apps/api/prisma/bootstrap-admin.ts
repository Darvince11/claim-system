...
async function main(){
  ...
  if(await db.user.count({where:{active:true,roles:{some:{role:{code:'ADMIN'}}}}}))throw new Error('An active administrator already exists. Use the administration interface to create additional users.');
