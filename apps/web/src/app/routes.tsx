...
import { Bell, Building2, FileText, ClipboardCheck, LayoutDashboard, LogOut, Menu, SearchCheck, UserRound, Users, X, BarChart3, GraduationCap, Banknote } from 'lucide-react';
...
function Shell(){
  const {user,logout}=useAuth();const [open,setOpen]=useState(false);const [error,setError]=useState('');const location=useLocation();const navigate=useNavigate();const [activeRole,setActiveRole]=useState(user!.roles[0]);
  useEffect(()=>{const routeRole=user!.roles.find(role=>location.pathname.startsWith(`/${rolePath[role]}/`));if(routeRole)setActiveRole(routeRole);},[location.pathname,user]);
  ...
  return <div className="workspace"><a className="skip-link" href="#workspace-main">Skip to content</a>{open&&<button className="nav-backdrop" aria-label="Close navigation" onClick={()=>setOpen(false)}/>}
    ...
    <nav aria-label="Workspace">{nav.map(({to,label,icon:Icon})=><NavLink key={to} to={to} onClick={()=>setOpen(false)}><Icon size={18}/>{label}</NavLink>)}</nav><div className="sidebar-account"><span className="avatar">{user!.name.split(' ').map(part=>part[0]).slice(0,2).join('')}</span><div><strong>{user!.name}</strong><small>{user!.staffId}</small></div><button className="icon-button" title="Sign out" aria-label="Sign out" onClick={async()=>{try{await logout();navigate('/login');}catch{setError('Signed out on this device.');}}}><LogOut size={18}/></button></div></aside>
