...
function Shell(){
  ...
  if(currentRole==='PRO_VC')nav.push({to:'/claims',label:'VC review queue',icon:FileText},{to:'/reports',label:'Approval reports',icon:BarChart3});
  ...
  if(currentRole==='ADMIN'){
    nav.push({to:'/admin/users',label:'Staff accounts',icon:Users},{to:'/admin/configuration',label:'Academic records',icon:Building2},{to:'/admin/claim-policy',label:'Claim policy',icon:ClipboardCheck},{to:'/audit/logs',label:'System audit logs',icon:SearchCheck});
  }
  nav.push({to:'/profile',label:'My profile',icon:UserRound});
