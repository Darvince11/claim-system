...
export async function downloadReport(type:string,format:string,query:string){
  const response=await fetch(apiUrl(`/reports/${type}/export?${query}&format=${format}`),{headers:{Authorization:`Bearer ${accessToken}`}});
